...
static int max8998_list_voltage_safeout(struct regulator_dev *rdev, unsigned int selector)
{
	int ldo = max8998_get_ldo(rdev);

	if (ldo == MAX8998_ESAFEOUT1 || ldo == MAX8998_ESAFEOUT2) {
		return 0;
	}
	return -EINVAL;
}
...
static struct regulator_ops max8998_safeout_ops = {
	.list_voltage           = max8998_list_voltage_safeout,
	.is_enabled             = max8998_ldo_is_enabled,
	.enable                 = max8998_ldo_enable,
	.disable                = max8998_ldo_disable,
	.get_voltage            = max8998_get_voltage,
	.set_voltage            = max8998_set_voltage_safeout,
	.set_suspend_enable     = max8998_ldo_enable,
	.set_suspend_disable    = max8998_ldo_disable,
};
...
static __devinit int max8998_pmic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		if (desc && regulators[index].ops != &max8998_others_ops
			&& regulators[index].ops != &max8998_safeout_ops) {
